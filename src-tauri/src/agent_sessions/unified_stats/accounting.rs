...
pub fn query_session_heatmap(
    ...
    let usage_by_session = session_usage_summaries(&response.sessions)?;
...
fn date_hour_from_timestamp(timestamp: &str, timezone_offset_minutes: i32) -> Option<(NaiveDate, u8)> {
    let parsed = chrono::DateTime::parse_from_rfc3339(timestamp).ok()?;
    let local = parsed.with_timezone(&chrono::Utc)
        - Duration::minutes(i64::from(timezone_offset_minutes));
