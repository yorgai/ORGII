...
pub fn list_opencode_history_sessions_paginated(
    ...
    imported_cache::query_imported_session_page(SOURCE_OPENCODE, limit, offset)
}

pub fn list_opencode_recent_paths(limit: usize) -> Result<Vec<OpenCodeRecentPath>, String> {
    sync_opencode_history_cache()?;
    imported_cache::query_imported_recent_paths(SOURCE_OPENCODE, limit)
}
...
fn sync_opencode_history_cache() -> Result<(), String> {
    let Some((conn, db_path)) = open_opencode_db()? else {
        imported_cache::sync_source_cache(SOURCE_OPENCODE, Vec::new(), Vec::new())?;
        return Ok(());
    };
    ...
    let inputs = metas
        ...
        .collect::<Vec<_>>();
    imported_cache::sync_source_cache(SOURCE_OPENCODE, live_ids, inputs)
}
