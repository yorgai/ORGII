...
pub fn cached_record_signatures(
    source: &str,
) -> Result<HashMap<String, ImportedHistoryRecordSignature>, String> {
    let conn = open_cache_conn()?;
    cached_record_signatures_from_conn(&conn, source)
}
...
pub fn upsert_imported_session_cache(inputs: &[ImportedHistoryCacheInput]) -> Result<(), String> {
    let mut conn = open_cache_conn()?;
    upsert_imported_session_cache_from_conn(&mut conn, inputs)
}
...
pub fn query_imported_session_page(
    source: &str,
    limit: usize,
    offset: usize,
) -> Result<ImportedHistorySessionPage, String> {
    let conn = open_cache_conn()?;
    query_imported_session_page_from_conn(&conn, source, limit, offset)
}
...
pub fn query_imported_recent_paths(
    source: &str,
    limit: usize,
) -> Result<Vec<ImportedHistoryRecentPath>, String> {
    let conn = open_cache_conn()?;
    query_imported_recent_paths_from_conn(&conn, source, limit)
}
...
pub fn get_cached_source_path(
    ...
    let conn = open_cache_conn()?;
    get_cached_source_path_from_conn(&conn, source, source_session_id)
}
...
{
    let cached = cached_record_signatures(source)?;
    Ok(discovered
        .iter()
        .filter(|record| {
...
fn open_cache_conn() -> Result<Connection, String> {
    get_connection().map_err(|err| format!("Failed to open imported history cache DB: {err}"))
}
