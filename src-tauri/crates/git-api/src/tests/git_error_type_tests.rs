...
fn from_git_error_not_a_git_repo() {
    let err = GitApiError::from_git_error("fatal: not a git repository (or any parent up to mount point /)");
    assert!(
...
fn from_git_error_could_not_read_username() {
    let err = GitApiError::from_git_error("remote: could not read Username for 'https://github.com'");
    assert!(
...
fn from_git_error_fallback_to_git_operation() {
    ...
    assert!(
        ...
        "Expected GitOperation fallback, got {:?}", err
    );
}
...
fn error_type_is_stable_for_all_variants() {
    let cases: Vec<(GitApiError, &str)> = vec![
        (
            GitApiError::RepoNotFound { repo_id: "r".into() },
            "repo_not_found",
        ),
        (
            GitApiError::RepoPathNotFound { path: PathBuf::from("/x") },
            "repo_path_not_found",
        ),
        (
            GitApiError::NotAGitRepo { path: PathBuf::from("/x") },
            "not_a_git_repo",
        ),
        (GitApiError::WatchManagerNotInitialized, "watch_manager_not_initialized"),
        (
            GitApiError::InvalidPath { path: "p".into(), reason: "r".into() },
            "invalid_path",
        ),
        (
            GitApiError::PathTraversal { path: "../x".into() },
            "path_traversal",
        ),
        (
            GitApiError::PathNotAllowed { path: "/etc".into() },
            "path_not_allowed",
        ),
        (
            GitApiError::GitOperation { message: "m".into() },
        ...
        (
            GitApiError::FileNotFoundAtRef { file_path: "f".into(), git_ref: "HEAD".into() },
            "file_not_found",
        ),
        (
            GitApiError::MergeConflict { message: "m".into(), files: vec![] },
            "merge_conflict",
        ),
        ...
        (
            GitApiError::RemoteNotFound { remote: "origin".into() },
            "remote_not_found",
        ),
        (
            GitApiError::AuthenticationFailed { remote: "origin".into() },
            "authentication_failed",
        ),
        (
            GitApiError::NonFastForward { message: "m".into() },
            "non_fast_forward",
        ),
        (
            GitApiError::NetworkError { message: "m".into() },
            "network_error",
        ),
        (
            GitApiError::InvalidRequest { message: "m".into() },
            "invalid_request",
        ),
        (
            GitApiError::InvalidEncoding { message: "m".into() },
            "invalid_encoding",
        ),
        (
            GitApiError::InvalidRef { git_ref: "bad".into() },
            "invalid_ref",
        ),
        (
            GitApiError::Internal { message: "m".into() },
            "internal_error",
        ),
...
fn uncommitted_changes_is_conflict() {
    assert_eq!(
        GitApiError::UncommittedChanges { files: vec!["foo.rs".into()] }.status_code(),
        StatusCode::CONFLICT
    );
}
...
fn non_fast_forward_is_conflict() {
    assert_eq!(
        GitApiError::NonFastForward { message: "rejected".into() }.status_code(),
        StatusCode::CONFLICT
    );
}
...
fn authentication_failed_is_unauthorized() {
    assert_eq!(
        GitApiError::AuthenticationFailed { remote: "origin".into() }.status_code(),
...
fn timeout_is_service_unavailable() {
    assert_eq!(GitApiError::Timeout.status_code(), StatusCode::SERVICE_UNAVAILABLE);
}

#[test]
fn nothing_to_commit_is_bad_request() {
    assert_eq!(GitApiError::NothingToCommit.status_code(), StatusCode::BAD_REQUEST);
}

#[test]
fn path_not_allowed_is_forbidden() {
    assert_eq!(
        GitApiError::PathNotAllowed { path: "/root/.ssh".into() }.status_code(),
