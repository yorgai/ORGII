...
use crate::intelligence::mcp::client::McpToolDef;
...
impl McpManager {
    ...
    pub(crate) async fn call_tool_with_meta(
        ...
        crate::intelligence::mcp::result::McpCallResult,
        crate::intelligence::mcp::errors::McpCallError,
    > {
        let client = {
            let clients = self.clients.lock().await;
            clients.get(server_name).cloned().ok_or_else(|| {
                crate::intelligence::mcp::errors::McpCallError::Transport {
        ...
        if !client.is_alive() {
            ...
            if let Err(err) = self.connect_server(server_name, &config).await {
                return Err(crate::intelligence::mcp::errors::McpCallError::Transport {
                    server: server_name.to_string(),
                    message: format!(
        ...
        if let Err(ref err) = outcome {
            ...
            if matches!(
                err,
                crate::intelligence::mcp::errors::McpCallError::Auth { .. }
            ) {
                let config = client.config().clone();
                if is_remote(&config) {
                    warn!(
                    ...
                    crate::intelligence::mcp::needs_auth_cache::set_entry(server_name).await;
