...
> **Version note**: Updated after the tool-schema/proxy incident (2026-04) where `schemars::openapi3()` injected a `$schema` URL that caused a proxy to inflate tokens from 4K to 93K. This was invisible to static code review and all 7 original layers. Added Layer 8 (Wire Protocol), Layer 9 (Init Parity), Layer 10 (Resolver Symmetry), and the "Systematic Sweep" discipline.
>
> **Updated 2026-04 (learnings-config refactor)**: Added anti-patterns 28–31 after `MemorySearchConfig` was found to bundle three unrelated domains (embedding, learnings policy, consolidation model override) under one misleading name. Root cause of `reflection-pipeline` E2E failure: background subsystems calling `ResolvedAgent::resolve()` which enforces `selected_model_id` — a requirement that doesn't apply to background jobs. Also added pattern for `expect()` on fallback paths that fail for the same reason as the primary path.
>
> **Updated 2026-05 (control-flow audit)**: Added anti-patterns for UI/runtime split-brain in control actions. A completed Rust turn can still be invisible if the authoritative EventStore write depends on a transient frontend broadcast listener; queue actions can pass DOM checks while bypassing the singleton dispatcher; cancel semantics must distinguish user Stop from programmatic Force Send. Rendered E2E must assert the effect, not just the click.
>
> **Updated 2026-05 (Agent Org finality audit)**: Added result-driven finality checks after an Agent Org run manually stopped at 4/6 completed tasks while `agent_org_runs.status` stayed `running` and open/ownerless work remained. Audit run/session/task state as separate dimensions; row/card existence is not scenario success. A valid test must fail if all materialized sessions are terminal while the run remains running, if `in_progress` has no owner, or if a completed run still has open work.
>
> **Updated 2026-05 (Agent Org task-runtime audit)**: Added role-aware task-tool and trajectory-cleanliness checks after `task_update(status=in_progress)` leaked `Execution failed: in_progress task must have an owner` into the Agent-visible trajectory. Multi-agent task tools must distinguish coordinator intent from member self-claim, return structured guidance for recoverable model mistakes, keep live task-board context out of stale prompt caches, and render task status as readable labels rather than ambiguous glyphs.
>
> **Updated 2026-05 (pre-user schema discipline)**: While the product has no external users or compatibility promise, schema changes should update the canonical `CREATE TABLE`/initialization DDL directly and reset local/E2E databases for verification. Do not add `ALTER TABLE`, legacy-table rebuilds, migration shims, or backward-compat tests unless the user explicitly asks for a persisted-data migration.
>
> **Updated 2026-05 (E2E false-path + control-record audit)**: Added checks after two regressions: an Agent Org rendered test used a debug drain endpoint to mark inbox rows read and then expected production chat history to change, and file-review polling treated `redo:rewind` control snapshots as pending user-review snapshots, clearing Redo state. Rendered tests must drive the production caller path, and UI registries must explicitly exclude control/sentinel records from user-actionable state.
>
> **Updated 2026-05 (Agent Org team-mode semantics audit)**: Added checks after Agent Org members sharing `builtin:sde` with the coordinator misidentified themselves, drained inbox messages were only ephemeral LLM context, and member sessions became terminal `completed` after each turn. Team-mode surfaces must treat runtime `member_id`/member name as identity, persist drained inbox as visible turn input before execution, and represent member turn completion as idle/available rather than session terminality.
>
> **Updated 2026-05 (Agent Org queue-progress audit)**: Added checks after a coordinator created UI-demo `in_progress` placeholder tasks and a dependency-unblocked assigned `pending` task remained stalled with no unread inbox. Task tools must enforce member-owned start semantics, must not notify blocked work early, and must redispatch `TaskAssigned` when dependencies become ready; E2E must assert inbox drain and ready-work progress, not just task-row rendering.
>
> **Updated 2026-06 (queue/turn lifecycle unification)**: Added anti-patterns 47–54 after a major refactor unified split-brain turn finality signals under a single FSM (`turnLifecycle.ts`). Root causes: multiple independent "is the turn done?" signals that could disagree and freeze or double-flush the queue; no generation counter to discard stale signals from old turns; the dispatcher duplicating FSM logic; cancel semantics conflated across user Stop and programmatic Force Send; a separate `holdSessionQueueForStopAtom` duplicating FSM state; turn finality derived from unreliable provider events instead of the FSM; duplicate send paths in UI components; and a multi-purpose cancel atom causing cross-concern bleed.
...
36. **"Control action has two send/cancel dispatchers"** (Added 2026-05) — Force Send appeared to work, but `ChatView` had a duplicate direct append + send path while `useQueueDispatch` already owned queued dispatch semantics. Double paths drifted on status, model selection, queue removal, cancel timing, and error handling. **Fix:** visible queue controls should only promote/cancel/request dispatch; exactly one dispatcher owns append, dequeue, model resolution, send, and error handling. **Detection rule:** for every visible control button, trace from click to side effect and ensure there is one source of truth for the mutation, not a UI shortcut plus a background dispatcher.

37. **"Cancel flag has one meaning for Stop and Force Send"** (Added 2026-05) — User Stop and programmatic Force Send both called cancel, but their semantics differ: Stop should restore/rollback and may mark the next turn as interrupted; Force Send should cut the current turn without poisoning the follow-up turn or leaking `[Request interrupted by user]`. Sharing one cancel flag/default branch caused the next Force Send turn to be cancelled immediately or to inherit synthetic interruption text. **Fix:** cancel APIs must carry intent (`user stop` vs `programmatic interrupt`) and runtime code must branch explicitly. **Detection rule:** when two UI controls call the same backend action, verify their postconditions are identical; if not, encode the difference in the API instead of relying on frontend timing or flag resets.
