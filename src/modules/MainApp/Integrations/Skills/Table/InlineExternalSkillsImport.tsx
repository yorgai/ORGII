...
function externalSkillRowKey(row: {
  sourceAgent: string;
  sourcePath: string;
...
const InlineExternalSkillsImport: React.FC<InlineExternalSkillsImportProps> = ({
  ...
  const {
    ...
    labels: { itemColumnHeader: t("skillsImport.itemColumn") },
  });

  const hasImportableSkills = allImportableItems.length > 0;

  return (
    <SectionContainer>
      <SectionRow label={t("skillsImport.title")}>
        <Button
          variant="secondary"
          icon={
            expanded ? (
          ...
          <div className="flex flex-col gap-3">
            {importLoading && items.length === 0 ? (
              <div className="rounded-md bg-fill-2 px-3 py-2 text-[12px] text-text-3">
                {t("skillsImport.scanning")}
              </div>
            ) : items.length === 0 ? (
              <div className="rounded-md bg-fill-2 px-3 py-2 text-[12px] text-text-3">
                {t("skillsImport.empty")}
              </div>
            ) : !hasImportableSkills ? (
              <div className="rounded-md bg-fill-2 px-3 py-2 text-[12px] text-text-3">
                {t("skillsImport.allImported")}
              </div>
            ) : (
              <SettingsTable
                columns={importColumns}
                rows={importableItems}
                getRowKey={externalSkillRowKey}
                headerHeight="tall"
                searchBar={{
                  searchValue: searchQuery,
                  onSearchChange: setSearchQuery,
                  searchPlaceholder: `${t("common:actions.search")} ${t("skillsImport.itemColumn").toLowerCase()}...`,
