...
| 3   | Select a worktree row                        | Breadcrumb shows `{branch}` only (no redundant folder prefix); loading placeholder appears immediately in the changes area; previous scope files are not visible |
...
| 12  | Rapid scope switching        | Click three different scopes quickly                     | Loading stays visible during transitions; final scope matches last selection without stale intermediate files    |
...
- [ ] Selecting a new scope shows loading in the changes/file list area on the same interaction (no stale flash)
