...
export function NewItemInput({
    ...
    (event: React.KeyboardEvent<HTMLInputElement>) => {
      switch (event.key) {
        case "Enter":
          ...
          event.stopPropagation(); // Prevent bubbling to parent (triggers rename)
          handleConfirm();
          break;
        case "Escape":
          event.preventDefault();
  ...
  const handleBlur = useCallback(() => {
    ...
    setTimeout(() => {
      if (!hasSubmittedRef.current && isMountedRef.current) {
        // For new items, blur with empty value cancels; blur with value confirms
        const trimmedValue = value.trim();
        if (trimmedValue) {
          handleConfirm();
        } else {
  ...
  return (
    ...
    >
      {/* Icon - chevron for folders, dynamic file type icon for files */}
      {isFolder ? (
