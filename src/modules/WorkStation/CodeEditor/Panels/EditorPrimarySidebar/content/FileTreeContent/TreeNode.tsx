...
const TreeNodeInner: React.FC<TreeNodeProps> = ({
  ...
  // Track current rename value for dynamic icon updates
  const [renameValue, setRenameValue] = useState(node.name);

  const isSelected = useIsFileSelected(node.path, node.name, repoPath);

  // In multi-root mode, both statusMap and folderStatusMap use absolute paths
  // as keys, so we look up directly by node.path. In single-root mode the
  // maps use relative paths, so we strip the repoPath prefix.
  const lookupPath = useMemo(() => {
    if (isMultiRoot) return node.path;
    if (!repoPath || !node.path.startsWith(repoPath)) return node.path;
    return node.path.substring(repoPath.length + 1);
  ...
  // Handle rename value change for dynamic icon updates
  ...
  if (isRenaming) {
    ...
    return (
      ...
      >
        {/* Icon - chevron for directories, dynamic file type icon for files */}
        {node.icon ? (
