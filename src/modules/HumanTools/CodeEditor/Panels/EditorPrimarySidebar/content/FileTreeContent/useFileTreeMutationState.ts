...
async function confirmKeyboardDelete(
  ...
  try {
    ...
    return askDialog(translate("confirmation.delete"), {
      title: translate("actions.confirmDeleteTitle", { name: fileName }),
      kind: "warning",
    ...
    return window.confirm(
      translate("actions.confirmDeleteTitle", { name: fileName })
    );
  }
}
      ...
      switch (event.key) {
        ...
        case "Backspace":
          if (event.metaKey || event.ctrlKey) {
            ...
            const confirmed = await confirmKeyboardDelete(fileName, t);
            if (!confirmed) break;
